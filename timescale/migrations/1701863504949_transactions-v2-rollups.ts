...
export const MAIN_TRANSACTION_TYPE = 'main'
const ROLLUPS = [
  {
    name: 'contract_transactions_v2_1m',
    ...
    customSelect: `
      ...
      GROUP BY bucket, blockchain, contract
    `
  },
  {
    name: 'contract_transactions_v2_10m',
    source: 'contract_transactions_v2_1m',
    interval: '10 minutes',
    startOffset: '1 day',
  ...
  {
    name: 'contract_transactions_v2_1h',
    source: 'contract_transactions_v2_10m',
    interval: '1 hour',
    startOffset: '3 days',
    endOffset: '2 hours',
    retention: '3 weeks'
  ...
  {
    name: 'contract_transactions_v2_1d',
    source: 'contract_transactions_v2_1h',
    interval: '1 day',
    startOffset: '6 days',
    endOffset: '1 day',
    retention: '30 days'
...
export async function up(pgm: MigrationBuilder): Promise<void> {
  for (const rollup of ROLLUPS) {
    const select = rollup?.customSelect || `
      ...
      FROM ${rollup.source}
      GROUP BY 1, blockchain, contract
    `
    ...
    // create indexes
    pgm.createIndex(rollup.name, [{ name: 'bucket', sort: 'DESC' }, 'blockchain', 'contract'])
